...
export default function LessonLayout({
    ...
    const lessonContentForQuiz = useMemo(() => {
        // Create a simple text representation of the lesson's core ideas for the AI.
        return `
            ${mainIdea}
            نتاجات التعلم: ${learningOutcomes.join(', ')}.
        `;
    }, [mainIdea, learningOutcomes]);

    useEffect(() => {
        if (currentUser) {
            localStorage.setItem('lastVisitedLesson', lessonId);
            // Mark lesson as complete if both interactive questions are answered correctly
            if (completedInteractiveCount >= 2) {
                markLessonAsComplete(currentUser.uid, lessonId);
            }
        }
    }, [lessonId, currentUser, completedInteractiveCount]);
        ...
        <div className="p-4 md:p-8 relative">
            ...
            <main className="space-y-8">
                ...
                <Card>
                    ...
                    <CardContent>
                        <Quiz lessonContent={lessonContentForQuiz} staticQuizzes={staticQuizzes} lessonId={lessonId} />
                    </CardContent>
                </Card>
            </main>
            ...
            <footer className="mt-12 border-t pt-6">
                <div className="flex flex-col-reverse md:flex-row md:justify-between gap-4">
                    {previousLesson ? (
                        <Link href={previousLesson} passHref className="w-full md:w-auto">
                    ...
                    {nextLesson && (
                         <Link href={nextLesson} passHref className="w-full md:w-auto">

...
export default function ProgressVessel({ label, percentage }: ProgressVesselProps) {
    ...
    // Calculate fill heights for each banner
    // Right banner fills from 0% to 50%
    const rightFillHeight = 50 * Math.min(1, p / 50); 
    // Left banner fills from 50% to 100%
    const leftFillHeight = p > 50 ? 50 * ((p - 50) / 50) : 0;
    ...
    return (
        <div className="relative w-full aspect-[3/4] text-center flex flex-col justify-end items-center" data-ai-hint="game castle progress">
            <svg viewBox="0 0 100 120" className="absolute inset-0 w-full h-full drop-shadow-md">
                 <defs>
                    {/* Clip path for the right banner, starts filling from the bottom */}
                    <clipPath id={`clip-right-banner-${label}`}>
                        <rect x="55" y={55 - rightFillHeight} width="20" height={rightFillHeight} />
                    </clipPath>
                    {/* Clip path for the left banner, starts filling from the bottom */}
                    <clipPath id={`clip-left-banner-${label}`}>
                         <rect x="30" y={55 - leftFillHeight} width="20" height={leftFillHeight} />
                    </clipPath>
                </defs>
                ...
                {/* Banners - Fill (Blue) - Apply clip paths here */}
